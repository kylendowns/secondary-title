<?php
	...
	function secondary_title_donation_notice() {
		...
		$deactivate_url = get_admin_url(get_current_blog_id(), "?secondary_title_show_donation_notice=off");
		$deactivate_url = wp_nonce_url($deactivate_url, "secondary_title_deactivate_donation_notice", "nonce");
		?>
		<div id="donation-notice" class="notice notice-info is-dismissible">
			...
			<p>
				<?php
					echo sprintf(
						__(
							"Ah, look at that, you are using my plugin %s. Excellent choice 😉",
							"secondary-title"
						),
						"<strong>Secondary Title</strong> (v" . SECONDARY_TITLE_VERSION . ")"
					);
				?>
			</p>
			<p>
				<?php
					_e(
						"If you want to make sure that the plugin continues to be updated to guarantee compatibility with future versions your plugins or themes, you can help by making a small donation. This will benefit not only yourself but the whole WordPress community!",
						"secondary-title"
					);
				?>
			</p>
			<div class="action-buttons">
				<p>
					...
					<a href="<?php echo esc_url($deactivate_url); ?>" class="button button-secondary dismiss-button">
	...
	function secondary_title_deactivate_donation_notice() {
		if(isset($_GET["secondary_title_show_donation_notice"], $_GET["nonce"]) && $_GET["secondary_title_show_donation_notice"] === "off" && wp_verify_nonce($_GET["nonce"], "secondary_title_deactivate_donation_notice")) {
			update_option(
